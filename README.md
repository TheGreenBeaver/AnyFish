...
- **Options**:
  ...
  `ResolveRace.TakeEvery` (_default_) results in every concurrent Promise updating the data, `ResolveRace.TakeFirst`
  waits for the first launched Promise to finish before accepting any new triggers, `ResolveRace.TakeLast` drops any
  running process whenever a new one is launched.  
